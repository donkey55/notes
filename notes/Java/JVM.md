...
- 对处理器资源非常敏感
- CMS收集器无法处理浮动垃圾（Floating Garbage），有可能出现Concurrent Mode Failure失败，而另一次完全的stw的full gc。在CMS并发标记和并发清理阶段，用户线程还是继续运行的，自然会有新的垃圾出现，但是这些垃圾无法在本次垃圾回收过程中被回收，只能等待下次的垃圾回收。这一部分垃圾被成为浮动垃圾。同时也因为在垃圾收集阶段用户线程还要持续运行，那就还要预留足够的内存空间给用户使用。所以CMS不能够等待老年代几乎被填满的情况下再收集，必须预留内存，供并发收集时程序使用。
